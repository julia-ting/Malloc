...
#define expandHeap my_sbrk(SBRK_SIZE)
#define metaSize sizeof(metadata_t)
...
metadata_t * mergeBuddy(int, int, metadata_t *one, metadata_t *buddy);
metadata_t * removeFromFreelist(int listIndex, metadata_t *block);
int my_log2(int);
metadata_t * checkBuddy(metadata_t *);
void addToFreelist(int, metadata_t *);
